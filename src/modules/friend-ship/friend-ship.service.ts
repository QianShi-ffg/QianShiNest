...
export class FriendShipService {
  ...
  create(createFriendShipDto: CreateFriendShipDto) {
    return this.FriendShipRepository.save({
      ...createFriendShipDto,
      status: createFriendShipDto.status || 'approved',
      hasBacklink: Boolean(createFriendShipDto.hasBacklink),
    });
  }

  apply(createFriendShipDto: CreateFriendShipDto) {
    return this.FriendShipRepository.save({
      ...createFriendShipDto,
      status: 'pending',
      hasBacklink: Boolean(createFriendShipDto.hasBacklink),
  ...
  findAll(query) {
    const { page, pageSize, sort = 'DESC', status } = query;
    const builder = this.FriendShipRepository.createQueryBuilder(
      'friend_ship',
    ).orderBy('friend_ship.id', sort === 'ASC' ? 'ASC' : 'DESC');

    if (status) {
      builder.andWhere('friend_ship.status = :status', { status });
    }
   ...
   * @returns
   */
  countFriendShip(query: any = {}) {
    const { status } = query;
    const builder = this.FriendShipRepository.createQueryBuilder('friend_ship')
      .select('COUNT(*) count')
    if (status) {
      builder.andWhere('friend_ship.status = :status', { status });
    }
